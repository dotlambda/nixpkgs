  ...
  coreFileSystemOpts = { name, config, ... }: {
    ...
    options = {
      ...
      after = mkOption {
        ...
        description = ''
          Mount points that should be mounted before this one.
          They have to be specified as <option>mountPoint</option> of some
          other file system.
        '';
      };
    };
...

{
  ...
  config = {
    ...
    assertions = let
      ...
      # Is mp' the mount point of a file system other than mp?
      notOtherMountPoint = mp: mp': mp == mp' || !(elem mp' (map (fs: fs.mountPoint) fileSystems));
      notOtherMountPoints = mp: mps: any (notOtherMountPoint mp) mps;
    in [
      ...
      {
        assertion = ! (any (fs: notOtherMountPoints fs.mountPoint fs.after) fileSystems);
        message = let
          fs = head (filter (fs: notOtherMountPoints fs.mountPoint fs.after) fileSystems);
          mp = head (filter (notOtherMountPoint fs.mountPoint) fs.after);
        in ''
          Mountpoint '${fs.mountPoint}': Entry '${mp}' of 'after' is not the mount points of another file system.
        '';
      }
    ];
